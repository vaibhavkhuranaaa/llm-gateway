...
| Containers | Non-root service targets; prior candidate scans had no high or critical findings | Point-in-time candidate images, not this undeployed source revision |
...
Five later managed simulator-only runs tested the same profile. The first three exposed capacity limits. After the API ceiling was raised from three to six instances, two fresh repetitions passed at 25.043 and 25.047 admitted requests per second with zero platform refusals. These results describe the verified hosted simulator revision, not the undeployed source tree in this repository.
